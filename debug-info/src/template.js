...
Handlebars.registerHelper("detail", (options) => {
  const {
    data: {
      root: {
        options: { detailInfo, detail },
      },
    },
  } = options;

  if (detail) {
    const { buildDate, currentBranchName, hashLastCommit } = detailInfo;
...
const compileTemplate = async (releases, details, options) => {
  ...
  if (handlebarsSetup) {
    const path = /^\//.test(handlebarsSetup)
      ? handlebarsSetup
      : join(process.cwd(), handlebarsSetup);
